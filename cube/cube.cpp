...
void demo_update_data_buffer(struct demo *demo) {
    ...
    mat4x4_look_at(demo->view_matrix, camera.lookAt, camera.origin, camera.up);
...
bool loadTexture(const char *filename, void* rgbav, VkSubresourceLayout *layout, uint32_t *width, uint32_t *height) {
    (void)filename;
    uint8_t *rgba_data = static_cast<uint8_t *>(rgbav);
    char *cPtr;
    ...
    for (i = 0; i < DEMO_TEXTURE_COUNT; i++) {
        ...
        if ((props.linearTilingFeatures & VK_FORMAT_FEATURE_SAMPLED_IMAGE_BIT) && !demo->use_staging_buffer) {
            ...
            demo_set_image_layout(demo, demo->textures[i].image, VK_IMAGE_ASPECT_COLOR_BIT, VK_IMAGE_LAYOUT_PREINITIALIZED,
                                  demo->textures[i].imageLayout, VK_ACCESS_NONE, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT,
...
static void demo_prepare_descriptor_layout(struct demo *demo) {
    const VkDescriptorSetLayoutBinding layout_bindings0 = {
            .binding = 0,
    ...
    const VkDescriptorSetLayoutBinding layout_bindings1 = {
            .binding = 1,
    ...
    VkDescriptorSetLayoutBinding layout_bindings[] = {layout_bindings0, layout_bindings1};
...
static void demo_prepare_render_pass(struct demo *demo) {
    ...
    const VkAttachmentDescription cAttachment = {
            .flags = 0,
            .format = demo->format,
            .samples = VK_SAMPLE_COUNT_1_BIT,
    ...
    const VkAttachmentDescription dAttachment = {
            .flags = 0,
    ...
    const VkAttachmentDescription attachments[] = {cAttachment, dAttachment};
    ...
    const VkSubpassDependency Depthd = {
    ...
    const VkSubpassDependency Depthd2  {
    ...
    VkSubpassDependency attachmentDependencies[2] = {Depthd, Depthd2};
...
static void demo_prepare_descriptor_pool(struct demo *demo) {
    const VkDescriptorPoolSize type_count0 =
    {
        .type = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER,
        .descriptorCount = demo->swapchainImageCount,
    };
    const VkDescriptorPoolSize type_count1 =
            {
                    .type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER,
                    .descriptorCount = demo->swapchainImageCount * DEMO_TEXTURE_COUNT,
            };
    const VkDescriptorPoolSize type_counts[2] = {type_count0, type_count1
    };
...
static void demo_prepare_descriptor_set(struct demo *demo) {
    ...
    writes[1].pImageInfo = &demo->offScreen->descriptorInfo;
...
static void demo_handle_xcb_event(struct demo *demo, const xcb_generic_event_t *event) {
    ...
    switch (event_code) {
        ...
        case XCB_MOTION_NOTIFY: {
            const xcb_motion_notify_event_t *cfg = (const xcb_motion_notify_event_t *)event;
            double normalX = (cfg->event_x / (float)demo->width) - 0.5f;
            double normalY = (cfg->event_y / (float)demo->height) - 0.5f;
            double normalZZ = 1.0f - normalX * normalX - normalY * normalY;
            double rr = normalX * normalX + normalY * normalY + normalZZ;
            double normalZ = sqrt(normalZZ);
            if (rr < 1.001f) {
                float newVec[3];
                newVec[0] = normalX;
                newVec[1] = normalY;
                newVec[2] = normalZ;
                ArcCamera::getCamera(camera.lookAt, camera.up,newVec);
...
static void demo_create_xcb_window(struct demo *demo) {
    ...
    value_list[1] = XCB_EVENT_MASK_KEY_RELEASE | XCB_EVENT_MASK_EXPOSURE | XCB_EVENT_MASK_STRUCTURE_NOTIFY | XCB_EVENT_MASK_POINTER_MOTION;
...
static void demo_init_vk(struct demo *demo) {
    ...
    VkBool32 validation_found = 0;
        ...
        if (instance_layer_count > 0) {
            ...
            validation_found = demo_check_layers(ARRAY_SIZE(instance_validation_layers), (char**)instance_validation_layers,
                                                 instance_layer_count, instance_layers);
            if (validation_found) {
                demo->enabled_layers.push_back("VK_LAYER_KHRONOS_validation");
            }
    ...
    if (instance_extension_count > 0) {
        ...
        for (uint32_t i = 0; i < instance_extension_count; i++) {
            if (!strcmp(VK_KHR_SURFACE_EXTENSION_NAME, instance_extensions[i].extensionName)) {
                surfaceExtFound = 1;
                demo->instanceExtensions.push_back(VK_KHR_SURFACE_EXTENSION_NAME);
            }
#if defined(VK_USE_PLATFORM_WIN32_KHR)
            ...
            if (!strcmp(VK_KHR_XCB_SURFACE_EXTENSION_NAME, instance_extensions[i].extensionName)) {
                platformSurfaceExtFound = 1;
                demo->instanceExtensions.push_back(VK_KHR_XCB_SURFACE_EXTENSION_NAME);
            }
#elif defined(VK_USE_PLATFORM_WAYLAND_KHR)
            ...
            if (!strcmp(VK_KHR_GET_PHYSICAL_DEVICE_PROPERTIES_2_EXTENSION_NAME, instance_extensions[i].extensionName)) {
                demo->instanceExtensions.push_back(VK_KHR_GET_PHYSICAL_DEVICE_PROPERTIES_2_EXTENSION_NAME);
            }
            if (!strcmp(VK_EXT_DEBUG_UTILS_EXTENSION_NAME, instance_extensions[i].extensionName)) {
                if (demo->validate) {
                    demo->instanceExtensions.push_back(VK_EXT_DEBUG_UTILS_EXTENSION_NAME);
                }
            }
            ...
            if (!strcmp(VK_KHR_PORTABILITY_ENUMERATION_EXTENSION_NAME, instance_extensions[i].extensionName)) {
                ...
                demo->instanceExtensions.push_back(VK_KHR_PORTABILITY_ENUMERATION_EXTENSION_NAME);
            }
        }
    ...
    for(uint32_t i = 0 ; i < demo->instanceExtensions.size(); ++i) {
        printf("%s\n", demo->instanceExtensions[i]);
    }
    VkInstanceCreateInfo inst_info = {
        .sType = VK_STRUCTURE_TYPE_INSTANCE_CREATE_INFO,
        .pNext = NULL,
        .flags = static_cast<VkInstanceCreateFlags>((portabilityEnumerationActive ? VK_INSTANCE_CREATE_ENUMERATE_PORTABILITY_BIT_KHR : 0)),
        .pApplicationInfo = &app,
        .enabledLayerCount = static_cast<uint32_t>(demo->enabled_layers.size()),
        .ppEnabledLayerNames = (const char *const *)instance_validation_layers,
        .enabledExtensionCount = static_cast<uint32_t>(demo->instanceExtensions.size()),
        .ppEnabledExtensionNames = (const char *const *)demo->instanceExtensions.data(),
    };
    ...
    if (device_extension_count > 0) {
        ...
        for (uint32_t i = 0; i < device_extension_count; i++) {
            if (!strcmp(VK_KHR_SWAPCHAIN_EXTENSION_NAME, device_extensions[i].extensionName)) {
                swapchainExtFound = 1;
                demo->deviceExtensions.push_back(VK_KHR_SWAPCHAIN_EXTENSION_NAME);
            }
            if (!strcmp("VK_KHR_portability_subset", device_extensions[i].extensionName)) {
                demo->deviceExtensions.push_back("VK_KHR_portability_subset");
            }
        }
        ...
        if (demo->VK_KHR_incremental_present_enabled) {
            ...
            for (uint32_t i = 0; i < device_extension_count; i++) {
                if (!strcmp(VK_KHR_INCREMENTAL_PRESENT_EXTENSION_NAME, device_extensions[i].extensionName)) {
                    demo->deviceExtensions.push_back(VK_KHR_INCREMENTAL_PRESENT_EXTENSION_NAME);
        ...
        if (demo->VK_GOOGLE_display_timing_enabled) {
            ...
            for (uint32_t i = 0; i < device_extension_count; i++) {
                if (!strcmp(VK_GOOGLE_DISPLAY_TIMING_EXTENSION_NAME, device_extensions[i].extensionName)) {
                    demo->deviceExtensions.push_back(VK_GOOGLE_DISPLAY_TIMING_EXTENSION_NAME);
...
static void demo_create_device(struct demo *demo) {
    ...
    queues[0].flags = 0;
    for(uint32_t i = 0 ; i < demo->deviceExtensions.size(); ++i) {
        printf("%s\n", demo->deviceExtensions[i]);
    }
    VkDeviceCreateInfo device = {
        ...
        .enabledExtensionCount = static_cast<uint32_t>(demo->deviceExtensions.size()),
        .ppEnabledExtensionNames = (const char *const *)demo->deviceExtensions.data(),
...
static void demo_init_vk_swapchain(struct demo *demo) {
    ...
    bundle = new VkBundle(demo->device, demo->graphics_queue, demo->graphics_queue_family_index, demo->gpu);
    demo->offScreen = new OffScreen(VK_FORMAT_R32G32_SFLOAT,VK_FORMAT_D32_SFLOAT, bundle);
    demo->offScreen->buildCommandBuffers();
}
...
static void demo_init(struct demo *demo, int argc, char **argv) {
    ...
    mat4x4_perspective(demo->projection_matrix, (float)degreesToRadians(70.f), 1.0f, 0.1f, 100.0f);
    mat4x4_look_at(demo->view_matrix, camera.lookAt, camera.origin, camera.up);
    mat4x4_identity(demo->model_matrix);
